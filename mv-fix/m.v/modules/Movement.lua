local function safeUI(n, f)
    return PrimeRuntime.Run("Movement/" .. n, f)
end
...
local nc = setmetatable({}, {__mode = "k"})
local function cleanupNoclip()
    for _, c in ipairs(noclipConnections) do c:Disconnect() end
    table.clear(noclipConnections)
    for o, v in pairs(nc) do if o.Parent then o.CanCollide = v end end
    table.clear(nc)
    currentCharacter = nil
end

local function setupNoclipForCharacter(c)
    if not noclipEnabled or not c then return end
    currentCharacter = c
    for _, o in ipairs(c:GetDescendants()) do
        if o:IsA("BasePart") then
            if nc[o] == nil then nc[o] = o.CanCollide end
            o.CanCollide = false
        end
    end
end
...
local function toggleNoclip(v)
    ...
    table.insert(noclipConnections, Player.CharacterAdded:Connect(setupNoclipForCharacter))
    table.insert(noclipConnections, game:GetService("RunService").Stepped:Connect(function()
...
TrackRuntimeCleanup(function() toggleAlwaysSprint(false) end)
TrackRuntimeCleanup(function() toggleAntiFling(false) end)
TrackRuntimeCleanup(function() toggleJumpPower(false) end)
TrackRuntimeCleanup(function() toggleWalkSpeed(false) end)
TrackRuntimeCleanup(function() toggleNoclip(false) end)
