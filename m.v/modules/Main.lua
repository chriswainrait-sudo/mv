...
local selectedCharacterChoice
local characterSelectorDropdown

local function getOwnedCharacters()
    local data = Player:FindFirstChild("PlayerData")
    local encoded = data and data:GetAttribute("OwnedCharacters")
    if type(encoded) ~= "string" then return {} end
    local ok, decoded = pcall(game:GetService("HttpService").JSONDecode, game:GetService("HttpService"), encoded)
    return ok and type(decoded) == "table" and decoded or {}
end

local function buildCharacterChoices()
    table.clear(characterChoices)
    local values = {}
    local owned = getOwnedCharacters()
    local folder = game:GetService("ReplicatedStorage"):FindFirstChild("CharacterData")
    if not folder then return values end
    for _, moduleScript in ipairs(folder:GetChildren()) do
        if moduleScript:IsA("ModuleScript") then
            local ok, data = pcall(require, moduleScript)
            if ok and type(data) == "table" then
                for skinKey, skin in pairs(data.Skins or {}) do
                    local purchase = type(skin) == "table" and skin.PurchaseData
                    local isFree = type(purchase) == "table" and purchase.Type == "Free"
                    local ownedSkins = owned[moduleScript.Name]
                    if isFree or (type(ownedSkins) == "table" and ownedSkins[skinKey]) then
                        local label = string.format("%s | %s", data.Name or moduleScript.Name, skin.Name or skinKey)
                        if characterChoices[label] then
                            label = label .. " (" .. skinKey .. ")"
                        end
                        characterChoices[label] = {Character = moduleScript.Name, Skin = skinKey}
...
local function refreshCharacterSelector()
    local values = buildCharacterChoices()
    if characterSelectorDropdown then
        characterSelectorDropdown:SetValues(values)
    end
    if #values > 0 and not characterChoices[selectedCharacterChoice] then
        selectedCharacterChoice = values[1]
        if characterSelectorDropdown then
            characterSelectorDropdown:SetValue(values[1])
        end
    end
end

local function invokeNetwork(name, ...)
    NetworkModule = NetworkModule or getGameModule("Network")
    if not NetworkModule then return false end
    local args = table.pack(...)
    for key, fn in pairs(NetworkModule) do
        if type(fn) == "function" and type(key) == "string" and string.find(key, "InvokeServer", 1, true) then
            local ok = pcall(fn, name, table.unpack(args, 1, args.n))
...
local function spawnSelectedCharacter()
    local choice = characterChoices[selectedCharacterChoice]
    if not choice then return false end
    local ok = invokeNetwork("RequestCharacter", choice.Character, choice.Skin)
    Fluent:Notify({
        Title = ok and "Character" or "Character Error",
        Content = ok and ("Selected " .. selectedCharacterChoice) or "Character request failed",
        Duration = 3
    })
    return ok
end
...
Tabs.Main:AddParagraph({
    ...
    local initialCharacterValues = buildCharacterChoices()
    selectedCharacterChoice = initialCharacterValues[1]
    characterSelectorDropdown = Tabs.Main:AddDropdown("CharacterSelector", {
        Title = "Character Selector",
        Description = "Select an owned character and skin",
        Values = initialCharacterValues,
        Multi = false,
        Default = selectedCharacterChoice
    })
    characterSelectorDropdown:OnChanged(function(value)
        selectedCharacterChoice = value
    end)
    Tabs.Main:AddButton({
        Title = "Spawn Selected Character",
        Description = "Spawn the selected character",
        Callback = spawnSelectedCharacter
    })
    Tabs.Main:AddButton({
        Title = "Refresh Characters",
        Description = "Refresh owned characters and skins",
        Callback = refreshCharacterSelector
    })
    local playerData = Player:FindFirstChild("PlayerData")
    if playerData then
        TrackRuntimeConnection(playerData:GetAttributeChangedSignal("OwnedCharacters"):Connect(refreshCharacterSelector))
    else
        task.spawn(function()
            local loadedData = Player:WaitForChild("PlayerData", 15)
            if loadedData and not PrimeRuntime.IsShuttingDown then
                TrackRuntimeConnection(loadedData:GetAttributeChangedSignal("OwnedCharacters"):Connect(refreshCharacterSelector))
