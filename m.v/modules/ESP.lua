local function safeUI(n, f)
    return PrimeRuntime.Run("ESP/" .. n, f)
end
...
function Visual.ToggleNoShadow(v)
    local l = game:GetService("Lighting")
    Visual.Effects.noShadowEnabled = v
    if v and not sh then
        sh = {v = l.GlobalShadows, lights = {}}
        for _, o in ipairs(l:GetDescendants()) do
            if o:IsA("Light") then sh.lights[o] = o.Shadows o.Shadows = false end
        end
        l.GlobalShadows = false
    elseif not v and sh then
        l.GlobalShadows = sh.v
        for o, x in pairs(sh.lights) do if o.Parent then o.Shadows = x end end
        sh = nil
    end
end

function Visual.ToggleNoFog(v)
    local l = game:GetService("Lighting")
    Visual.Effects.noFogEnabled = v
    if v and not fog then
        fog = {l.FogStart, l.FogEnd, a = {}}
        for _, o in ipairs(l:GetChildren()) do
            if o:IsA("Atmosphere") then fog.a[o] = o.Density o.Density = 0 end
        end
        l.FogStart, l.FogEnd = 0, 10000000
    elseif not v and fog then
        l.FogStart, l.FogEnd = fog[1], fog[2]
        for o, x in pairs(fog.a) do if o.Parent then o.Density = x end end
        fog = nil
    end
end

function Visual.ToggleSaturation(v)
    Visual.Effects.saturationEnabled = v
    if v then Visual.UpdateSaturation()
    elseif sat then sat:Destroy() sat = nil end
end

function Visual.UpdateSaturation()
    if not Visual.Effects.saturationEnabled then return end
    if not sat or not sat.Parent then
        sat = Instance.new("ColorCorrectionEffect")
        sat.Name = "MV_Saturation"
        sat.Parent = game:GetService("Lighting")
    end
    sat.Saturation = math.clamp(Visual.Effects.saturationLevel / 5, -1, 1)
end
...
TrackRuntimeCleanup(function() Visual.Cleanup() end)
TrackRuntimeCleanup(function() cleanupCombatESP() end)
